...
const cardStyle = computed(() => {
  if (!isHovered.value || !cardRef.value) return 'transform: perspective(1000px) rotateX(0deg) rotateY(0deg) scale3d(1, 1, 1);';
  ...
  return `transform: perspective(1000px) rotateX(${rotateX}deg) rotateY(${rotateY}deg) scale3d(1.02, 1.02, 1.02); transition: transform 0.1s;`;
});
...
<template>
  <div 
    ref="cardRef"
    class="hitokoto-container cyber-glass w-full h-[320px] relative cursor-pointer select-none group"
